...
**이 `docs/` 폴더(00~06번 절차 문서 7개 + `QA_*.md` 5개 + 결과 기록용
`PoC_Test_Results.xlsx`, 총 13개 파일)만으로 테스트를 처음부터 끝까지 수행할 수
있습니다.**
...
[PoC_Test_Results.xlsx](PoC_Test_Results.xlsx) — 33개 TC의 결과를 실제로 기입하는
엑셀 파일입니다. 시트 7개로 구성:

| # | 시트명 | 내용 |
|---|---|---|
| 1 | `1_개요` | PoC 목적, 배경/제약, 테스트 범위·환경·기간, 카테고리별 PASS/FAIL 자동 집계표(수식) |
| 2 | `2_샘플결과` | 열 설명(결과/실제 결과/해석 각각 무엇을 적어야 하는지) + PASS 2건·FAIL 1건 작성 예시 |
| 3~7 | `3_기능검증` ~ `7_운영성능` | 01~06 문서의 카테고리(FUNC/DATA/RBAC/SEC/PERF)별로 1시트씩, TC 코드·테스트명·중요도·합격 기준은 이미 채워져 있고 수행일·담당자·결과·실제 결과·해석만 입력하면 됨 |

"결과" 열은 PASS/FAIL/BLOCKED/N/A 드롭다운이고 값에 따라 셀 색이 자동으로 바뀝니다
(초록/빨강/노랑/회색). `1_개요` 시트의 집계표는 3~7번 시트의 결과 열을 `COUNTIF`로
자동 계산하므로 별도로 손댈 필요가 없습니다.
...
각 문서에서 얻은 TC별 결과는 [PoC_Test_Results.xlsx](PoC_Test_Results.xlsx)에 취합합니다
(구성은 위 "결과 기록 워크북" 참고). API 제공 및 RAG 범위 포함 결정 이후에는 이 워크북의
결과와 나머지 17개 TC(RAG 전체 7개, TC-FUNC-02, TC-DATA-08, TC-PERF-06, TC-API-01~07)
결과를 합쳐 50개 전체 결과표를 완성하면 됩니다.

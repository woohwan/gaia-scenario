...
실행하는 데 필요한 데이터를 준비한 폴더입니다. **원본(cohesity-poc의 `gaia_dataset/`,
`gaia_web_dataset/`)은 전혀 수정하지 않았고**, 필요한 만큼만 복사하거나 신규 합성했습니다.
신규 합성 데이터는 전부 완전 가상/합성이며 실존 인물·기업 정보가 아닙니다.

이 폴더(및 상위 저장소)는 cohesity-poc 밖으로 옮겨도 동작하도록 만들어졌습니다
(`gaia_eval`과 동일한 패턴 — 아래 "절대경로 설정" 참고).

## 절대경로 설정 (`paths_config.py`)
대용량 원본은 복제하지 않고 절대경로로 직접 가리킵니다. 기본값은 `paths_config.py`에
정의되어 있고, 환경 변수로 오버라이드할 수 있습니다 (cohesity-poc가 다른 위치로 이동한
경우 등):

| 변수 | 기본값 |
|---|---|
| `GAIA_DATASET_DIR` | `/data/richard/cohesity-poc/gaia_dataset` |
| `GAIA_WEB_DATASET_DIR` | `/data/richard/cohesity-poc/gaia_web_dataset` |
| `COLLECTOR_EVAL_OUTPUT_DIR` | `/data/richard/cohesity-poc/collector/eval/output` |
| `GAIA_RAGAS_EVAL_DIR` | `/data/richard/cohesity-poc/gaia_ragas/eval` |
...
| RAG | TC-RAG-01~07 | `TC-RAG/` | gaia_ragas 기존 QA 200개 재샘플링 + 무응답/모호질문 신규 작성 (질문지는 독립, 실채점은 `$GAIA_DATASET_DIR` 필요) |
| DATA | TC-DATA-01 | `TC-DATA-01_format_coverage/` | 동일 내용 8포맷 신규 합성(독립) + `$COLLECTOR_EVAL_OUTPUT_DIR` 기존 포맷별 QA 참조 |
...
## 원칙
- cohesity-poc의 원본 경로는 read-only로만 참조했고, 어떤 파일도 수정·삭제하지
  않았습니다.
- 큰 원본 데이터를 통째로 복제하지 않고, 케이스별로 필요한 최소량만 추출하거나
  (FUNC 1개사 123MB) 아예 새로 합성했습니다(RBAC용 HR 문서, SEC 인젝션 문서,
  DATA-01/07 fixture 등).
- 대용량 성능 티어(TC-DATA-06, TC-PERF-03)와 골든셋 실채점 대상 문서는 복사 대신
  `paths_config.py`의 절대경로로 원본을 직접 가리키는 방식으로 처리했습니다
  (불필요한 디스크 사용 방지, cohesity-poc 원본이 삭제/이동되지 않는 한 계속 유효).
